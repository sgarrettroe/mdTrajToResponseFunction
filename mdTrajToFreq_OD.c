...
#define DT 0.01 //ps, time-steps output by GROMACS
#define FUDGE 0.2659 // 0.188 * sqrt(2) to reduce the frequecies from OH to OD

int main(int iarg, char **input) 
{
  time_t my_time=time(0); // time process started
  FILE *coord_fid,*force_fid,*param_fid,*fid;
  char name[100],coord_file_name[100],force_file_name[100],base_name[100],string[100];
  ...
  if(iarg<4)
    {
      ...
      printf("USAGE:\n   %s coordinate_file force_file output_file_prefix\n\n",input[0]);
      exit(EXIT_FAILURE);
    }
    else
    {	
      strcpy(coord_file_name,input[1]);
      strcpy(force_file_name,input[2]);
    ...
    for(i=1;i<=nsteps;i++)
    {
      ...
      for(j=1;j<=nmols;j++)
      {
        ...
        // calculate mass weighted forces
        for(k=1;k<=3;k++)
          force[k] = (forceH2[k]/mH - forceO[k]/mO)*mu;
