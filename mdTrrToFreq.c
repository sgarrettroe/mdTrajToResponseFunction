...
//#define DT 0.01 //ps, time-steps output by GROMACS //read from mdp.file
#define FUDGE 0.188 //ask Peter -- this scales the forces up

int main(int iarg, char **input) 
{
  time_t my_time=time(0); // time process started
  FILE *trr_fid,*param_fid,*fid;
  char name[100],mdp_file_name[100],trr_file_name[100],base_name[100],string[100];
  int idummy;

  const float NA = 6.023e23; // mol^-1
  const float Anh = 270; //cm-1
  const float w0 = 3450; // cm-1
  ...
  float fact; //scaling factor for forces to frequency
  float mu; //reduced mass
  
  //vars for force trajectory
  float dummy;
  unsigned long nsteps, natoms, nmols, nprotons, nstxout,nstfout,nstvout;
  float dt=0;
  ...
  if(iarg<4)
    {
      ...
      printf("USAGE:\n   %s mdp_file trr_file output_file_prefix\n\n",input[0]);
      exit(EXIT_FAILURE);
    }
    else
    {	
      strcpy(mdp_file_name,input[1]);
      strcpy(trr_file_name,input[2]);
      strcpy(base_name,input[3]);
      
      printf("Writing parameter file\n");
      strcpy(name,base_name);
      strcat(name,"_parameters.txt");
      param_fid = fopen(name,"wt");
      fprintf(param_fid,"This is %s called as\n",input[0]);
      fprintf(param_fid,"%s %s %s %s\n",input[0],input[1],input[2],input[3]);
      fprintf(param_fid,"On: %s\n",ctime(&my_time));
      fprintf(param_fid,"\n"); 
    }
    
    
    printf("Determine dt, number of steps, and number of molecules from mdp file.\n");
    strcpy(string,"grep dt ");
    strcat(string,mdp_file_name);
    fid = popen(string,"r");
    if(fid==NULL) nrerror("Opening 'grep dt mdp_file_name' pipe failed.");
    idummy=fscanf(fid,"dt              =%ld",&nsteps);
    pclose(fid);
    if (idummy!=1) nrerror("Failed to find dt!");
    strcpy(string,"grep nsteps ");
    strcat(string,mdp_file_name);
    fid = popen(string,"r");
    if(fid==NULL) nrerror("Opening 'grep nsteps mdp_file_name' pipe failed.");
    fscanf(fid,"nsteps              =%ld",&nsteps);
    pclose(fid);
    strcpy(string,"grep nstxout ");
    strcat(string,mdp_file_name);
    fid = popen(string,"r");
    if(fid==NULL) nrerror("Opening 'grep nstxout mdp_file_name' pipe failed.");
    fscanf(fid,"nstxout             =%ld",&nstxout);
    pclose(fid);
    strcpy(string,"grep nstvout ");
    strcat(string,mdp_file_name);
    fid = popen(string,"r");
    if(fid==NULL) nrerror("Opening 'grep nstvout mdp_file_name' pipe failed.");
    fscanf(fid,"nstvout             =%ld",&nstvout);
    pclose(fid);
    strcpy(string,"grep nstfout ");
    strcat(string,mdp_file_name);
    fid = popen(string,"r");
    if(fid==NULL) nrerror("Opening 'grep nstfout mdp_file_name' pipe failed.");
    fscanf(fid,"nstfout             =%ld",&nstfout);
    pclose(fid);
	...
	printf("nstxout = %ld\n",nstxout);
	printf("nstfout = %ld\n",nstfout);
	nrerror("ERROR: md parameters must output coordinates and forces at the same time -- i.e. nstxout = nstfout");
      }

    printf("Found %ld total steps from file %s\n", nsteps, mdp_file_name);
    nsteps=nsteps/nstxout+1;
    printf("Print coordinates and forces every %ld steps from file %s\n", nstxout, mdp_file_name);
    printf("Print velocities every %ld steps from file %s\n", nstvout, mdp_file_name);
    nstvout=nstvout/nstxout; //from now on, only the ratio matters

    lineptr= (char *) malloc(sizeof(char)*n_line);
    strcpy(string,"/usr/local/bin/gmxdump -f ");
    strcat(string,trr_file_name);
    trr_fid=popen(string,"rt");
    if(trr_fid==NULL) 
      {
	printf(string);
	fflush(NULL);
	nrerror("Opening pipe 'gmxdump -f trr_file' failed. (1)"); //why does this break?
      }

    //read line 1 of the trr file and ignore it
    fgets(lineptr,n_line,trr_fid);
    line_count+=1;
    //read line 2
    fgets(lineptr,n_line,trr_fid);
    line_count+=1;
    //search for the number of atoms
    sscanf(lineptr,"   natoms=%ld",&natoms);
    fclose(trr_fid);
    line_count=0;

    nmols = natoms/3; //3 atoms per H2O molecule
    nprotons = nmols*2;//2 protons
    printf("Found %ld atoms, %ld molecules in file %s\n", natoms, nmols, trr_file_name);
    ...
    fprintf(param_fid,"fact = %f\n",fact);
    fprintf(param_fid,"FUDGE = %f",FUDGE);
    fprintf(param_fid,"\n"); 

    posO    = matrix(1,nmols,1,3);
    posH1   = matrix(1,nmols,1,3);
    posH2   = matrix(1,nmols,1,3);
    forceO  = matrix(1,nmols,1,3);
    forceH1 = matrix(1,nmols,1,3);
    forceH2 = matrix(1,nmols,1,3);
    bond    = vector(1,3);
    force   = vector(1,3);
    ...
    strcpy(string,"gmxdump -f ");
    strcat(string,trr_file_name);
    trr_fid=popen(string,"rt");
    if(trr_fid==NULL) nrerror("Opening pipe 'gmxdump -f trr_file' failed.");
    

    /*
     * loop over timesteps storing only the frequency of each OH stretch
     */
    int dummyint;
    #define NDEBUGSTEPS 0
    for(i=1;i<=nsteps;i++)
    {
      //skip header lines
      for(j=1;j<=7;j++)
	fgets(lineptr,n_line,trr_fid);
      if (i<=NDEBUGSTEPS)
	{
	  printf("Next line should say 'x (3057x3)'\n");
	  printf(lineptr);
	}
	...
	{
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      x[%d]={%e, %e, %e}",
		    &dummyint,&posO[j][1],&posO[j][2],&posO[j][3])!=4)
	    {
	      printf("crashing...\n");
	      printf("%ld %ld",i,j);
	      printf("%s",lineptr);
	      nrerror("failed to read posO before end of file");
	    }
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      x[%d]={%e, %e, %e}",
		    &dummyint,&posH1[j][1],&posH1[j][2],&posH1[j][3])!=4) 
	    nrerror("failed to read posH1 before end of file");
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      x[%d]={%e, %e, %e}",
		    &dummyint,&posH2[j][1],&posH2[j][2],&posH2[j][3])!=4) 
	    nrerror("failed to read posH2 before end of file");
	}

      //Only look for velocities every nstvout steps
      if ( (i%nstvout)==1)
	{
	  // header line (e.g. v (3057x3); )
	  fgets(lineptr,n_line,trr_fid);
	  if(i<=NDEBUGSTEPS)
	    {
	      printf("Next line should say 'v (3057x3)'\n");
	      printf(lineptr);
	    }
      ...
      // read all the forces
      // header line (e.g. f (3057x3); )
      fgets(lineptr,n_line,trr_fid);
      if (i<=NDEBUGSTEPS)
	{
	  printf("Next line should say 'f (3057x3)'\n");
	  printf(lineptr);
	}
      for(j=1;j<=nmols;j++)
	{
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      f[%d]={%e, %e, %e}",
		    &dummyint,&forceO[j][1],&forceO[j][2],&forceO[j][3])!=4)
	    {
	      printf("crashing...\n");
	      printf("%ld %ld",i,j);
	      printf("%s",lineptr);
	      nrerror("failed to read forceO before end of file");
	    }
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      f[%d]={%e, %e, %e}",
		    &dummyint,&forceH1[j][1],&forceH1[j][2],&forceH1[j][3])!=4) 
	    nrerror("failed to read forceH1 before end of file");
	  fgets(lineptr,n_line,trr_fid);
	  if(sscanf(lineptr,"      f[%d]={%e, %e, %e}",
		    &dummyint,&forceH2[j][1],&forceH2[j][2],&forceH2[j][3])!=4) 
	    nrerror("failed to read forceH2 before end of file");
	}

      //calculate bond vectors and finally frequencies
      for(j=1;j<=nmols;j++)
	{
	  // calculate the bond vector and bond length for H1
	  bond_length=0;
	  for(k=1;k<=3;k++)
	    bond[k] = posH1[j][k] - posO[j][k];
	  bond_length=sqrt(bond[1]*bond[1] + bond[2]*bond[2] + bond[3]*bond[3]);
	  
	  // calculate mass weighted forces
	  for(k=1;k<=3;k++)
	    force[k] = (forceH1[j][k]/mH - forceO[j][k]/mO)*mu;
	  
	  //calculate projection of forces on H1
	  dw_matrix[2*j-1][i]=0;
	  for(k=1;k<=3;k++)
	    dw_matrix[2*j-1][i] -= force[k]*bond[k];
        
	  // scale field to effective OH stretch frequency
	  dw_matrix[2*j-1][i] *= fact/bond_length/FUDGE;
	  
	  // calculate the bond vector and bond length for H2
	  bond_length=0;
	  for(k=1;k<=3;k++)
	    bond[k] = posH2[k] - posO[k];
	  bond_length=sqrt(bond[1]*bond[1] + bond[2]*bond[2] + bond[3]*bond[3]);
	  
	  // calculate mass weighted forces
	  for(k=1;k<=3;k++)
	    force[k] = (forceH2[j][k]/mH - forceO[j][k]/mO)*mu;
	  
	  //calculate projection of forces on H2
	  dw_matrix[2*j][i]=0;
	  for(k=1;k<=3;k++)
	    dw_matrix[2*j][i] -= force[k]*bond[k];
	  
	  // scale field to effective OH stretch frequency
	  dw_matrix[2*j][i] *= fact/bond_length/FUDGE;
	}
    }
    fclose(trr_fid);
    
    printf("Appending %ld step frequency trajectories\n",nsteps);
    strcpy(name,base_name);
    strcat(name,"_dw.dat");
    fid = fopen(name,"at");
    if(fid==NULL) nrerror("Failed opening frequency file (basename_dw.dat) for output.");
    //strcpy(name,"bzip2 -c >> ");
    //strcat(name,base_name);
    //strcat(name,"_dw.dat.bz2");
    //fid = popen(name,"w");
    //for(i=1;i<=nprotons;i++){
    //  for(j=1;j<=dummy;j++)
    //    fprintf(fid,"%8.5f ",dw_matrix[i][j]);
    //  fprintf(fid,"\n");
    //}
    for(j=1;j<=nsteps;j++){
        for(i=1;i<=nprotons;i++)
	  fprintf(fid,"%12.3f",dw_matrix[i][j]);
      fprintf(fid,"\n");
    }
    fclose(fid);

    // printf("...positions and forces\n");
    free(lineptr);
    free_matrix(posO,1,nmols,1,3);
    free_matrix(posH1,1,nmols,1,3);
    free_matrix(posH2,1,nmols,1,3);
    free_matrix(forceO,1,nmols,1,3);
    free_matrix(forceH1,1,nmols,1,3);
    free_matrix(forceH2,1,nmols,1,3);
